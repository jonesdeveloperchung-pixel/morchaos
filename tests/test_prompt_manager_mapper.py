...
def test_map_prompt_files_with_output(tmp_path):
    ...
    assert loaded_mapping == [{"nickname": "test", "full_path": "prompts_test.json"}]
    assert mappings == [{"nickname": "test", "full_path": "prompts_test.json"}]
...
def test_download_prompt_from_docsbot_failure(tmp_path, mocker):
    ...
    output_dir = tmp_path / "downloads"
    downloaded_file = prompt_manager.download_prompt_from_docsbot(
        "https://docsbot.ai/prompts/fail", output_dir
    )
...
def test_cli_convert_command(mock_convert_prompt_format, mocker):
    ...
    mock_convert_prompt_format.assert_called_once_with(
        Path("input.txt"), Path("output.json")
    )
...
def test_cli_map_command_custom_paths(mock_map_prompt_files, mocker):
    ...
    cli_prompt_manager.main()
    mock_map_prompt_files.assert_called_once_with(
        Path("my_prompts_folder"), Path("custom_map.json")
    )
...
def test_cli_download_command_success(
    mock_download_prompt_from_docsbot, mocker, capsys
):
    ...
    mocker.patch("sys.exit")
    cli_prompt_manager.main()
    mock_download_prompt_from_docsbot.assert_called_once_with(
