...
def test_ollama_chat_inline_prompts(mock_run_chat_core, mocker):
    mocker.patch('sys.argv', [
        'ollama-chat',
        '-S', 'You are a sarcastic Unix expert.',
        '-U', 'Explain virtual memory.',
    ])
    ollama_chat.main()
    mock_run_chat_core.assert_called_once()
    call_args, call_kwargs = mock_run_chat_core.call_args
    messages = call_args[0]
    assert messages[0] == {'role': 'system', 'content': 'You are a sarcastic Unix expert.'}
...
def test_ollama_chat_file_based_system_prompt(tmp_path, mock_run_chat_core, mocker):
    ...
    mocker.patch('sys.argv', [
        'ollama-chat',
        '--sf', str(system_prompt_file),
        '-U', 'What is a closure in Python?',
    ])
    ollama_chat.main()
    mock_run_chat_core.assert_called_once()
    call_args, call_kwargs = mock_run_chat_core.call_args
...
def test_ollama_chat_default_system_prompt(mock_run_chat_core, mocker):
    mocker.patch('sys.argv', [
        'ollama-chat',
        '-U', 'Show me the Linux kernel source.',
    ])
    ollama_chat.main()
    mock_run_chat_core.assert_called_once()
    call_args, call_kwargs = mock_run_chat_core.call_args
    ...
    assert messages[1] == {'role': 'user', 'content': 'Show me the Linux kernel source.'}
    assert call_kwargs['model'] == 'gemma3:4b'
...
def test_health_check(mock_health_check_core, mocker, capsys):
    ...
    mock_health_check_core.assert_called_once_with('http://localhost:11434', 30) # Default timeout
...
def test_list_models(mock_list_models_core, mocker, capsys):
    ...
    mock_list_models_core.assert_called_once_with('http://localhost:11434', 30) # Default timeout
