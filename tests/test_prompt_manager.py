...
def test_read_prompt_string():
    ...
    prompt = prompt_manager.read_prompt(
        source=source, file_path=None, prompt_name="test"
    )
...
def test_read_prompt_txt_file(tmp_path):
    ...
    prompt = prompt_manager.read_prompt(
        source=None, file_path=file_path, prompt_name="test"
    )
...
def test_read_prompt_json_file(tmp_path):
    ...
    prompt = prompt_manager.read_prompt(
        source=None, file_path=file_path, prompt_name="system"
    )
...
def test_read_prompt_default():
    ...
    prompt = prompt_manager.read_prompt(
        source=None, file_path=None, prompt_name="test", default=default_prompt
    )
...
def test_read_prompt_missing_file_raises_error():
    with pytest.raises(ValueError, match="Test file 'non_existent.txt' not found."):
        prompt_manager.read_prompt(
            source=None, file_path=Path("non_existent.txt"), prompt_name="test"
        )
