...
async def run_chat(
    ...
    try:
        ...
        logger.error(
            f"Ollama chat failed with model {model} at {url}. Exception: {exc}",
            exc_info=True,
        )
        raise RuntimeError(
            f"Ollama chat failed with model {model} at {url}. See logs for details."
        ) from exc
...
def list_models(
    url: str = "http://localhost:11434", timeout: int = 30
) -> list[dict[str, any]]:
